...
function Test-PreviewHealth {
  try {
    ...
    if (-not $check.Healthy) {
      Write-MonitorLog "Health details: rails=$($check.RailsRunning), tunnel=$($check.TunnelRunning), environment=$($check.ServerEnvironment), isolated_database=$($check.IsolatedDatabase), local=[$($check.LocalMarkets); $($check.LocalApplication)], public=[$($check.PublicMarkets); $($check.PublicApplication)]."
    }
